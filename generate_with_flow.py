...
def sample_sequence(length_input, model, context, length, n_ctx, tokenizer, temperature=1.0, top_k=30, top_p=0.0, repitition_penalty=1.0,
    ...
    with torch.no_grad():
        while 1:
            ...
            if text[-1] == ',' or text[-1] == '。' or text[-1] == '[CLS]' or text[-1] == '[SEP]' or text[-1] == '?' or text[-1] == '!' or text[-1] == '，' or text[-1] == '.' or text[-1] == '：' or text[-1] == ':':
...
def main():
    ...
    parser.add_argument('--prefix', default='我忘不了', type=str, required=False, help='生成文章的开头')
